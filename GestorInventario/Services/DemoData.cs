...
{
    /// <summary>
    /// In-memory demo data — replace with real DB calls via Repositories.
    /// </summary>
    public static class DemoData
    {
        ...
        public static List<MovimientoInventario> Movimientos = new()
        {
            new() { Id=1, ProductoId=1, Producto="Monitor Samsung 24\"", TipoMovimientoId=1, TipoMovimiento="Entrada", Cantidad=10, Observacion="Compra inicial", UsuarioId=1, Usuario="Administrador", ProveedorId=1, Proveedor="TechSupply S.A.", Fecha=DateTime.Today.AddHours(-3) },
            new() { Id=2, ProductoId=2, Producto="Teclado Mecánico", TipoMovimientoId=2, TipoMovimiento="Salida", Cantidad=2, Observacion="Venta departamento TI", UsuarioId=1, Usuario="Administrador", Fecha=DateTime.Today.AddHours(-2) },
            new() { Id=3, ProductoId=3, Producto="Cable HDMI 2m", TipoMovimientoId=1, TipoMovimiento="Entrada", Cantidad=30, Observacion="Reabastecimiento mensual", UsuarioId=1, Usuario="Administrador", ProveedorId=2, Proveedor="Distribuidora Norte", Fecha=DateTime.Today.AddDays(-1) },
            new() { Id=4, ProductoId=4, Producto="Mouse Inalámbrico", TipoMovimientoId=2, TipoMovimiento="Salida", Cantidad=5, Observacion="Préstamo sala reuniones", UsuarioId=1, Usuario="Administrador", Fecha=DateTime.Today.AddDays(-2) },
        };

        public static List<Alerta> Alertas = new()
        {
            new() { Id=1, ProductoId=2, Producto="Teclado Mecánico", StockActual=3, StockMinimo=5, Estado="Crítico", CreadoEn=DateTime.Now.AddHours(-1) },
            new() { Id=2, ProductoId=4, Producto="Mouse Inalámbrico", StockActual=2, StockMinimo=8, Estado="Crítico", CreadoEn=DateTime.Now.AddHours(-2) },
            new() { Id=3, ProductoId=1, Producto="Monitor Samsung 24\"", StockActual=12, StockMinimo=5, Estado="Normal", CreadoEn=DateTime.Now.AddDays(-1) },
        };
    }
}
